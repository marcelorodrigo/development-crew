...
const lockFileSystem = { mkdir, readdir, readFile, rm, rmdir, stat, utimes, writeFile };
...
export async function acquireLock(wrapperDirectory, fileSystem = lockFileSystem) {
  const lockDirectory = `${wrapperDirectory}.development-crew-update.lock`;
  const breakerDirectory = `${lockDirectory}.breaker`;
  const token = randomUUID();
  const generationDirectory = join(lockDirectory, token);
  const ownerPath = join(generationDirectory, 'owner');
  const heartbeatPath = join(generationDirectory, 'heartbeat');
  ...
  for (;;) {
    ...
    try {
      ...
      created = true;
      await fileSystem.mkdir(generationDirectory);
      await fileSystem.writeFile(ownerPath, token, { encoding: 'utf8', flag: 'wx' });
      await fileSystem.writeFile(heartbeatPath, '', { flag: 'wx' });

      const heartbeat = setInterval(() => {
        const now = new Date();
        void fileSystem.utimes(heartbeatPath, now, now).catch(() => {});
      }, 10_000);
      heartbeat.unref?.();
      ...
      return async () => {
        ...
        let owner;
        try {
          owner = (await fileSystem.readFile(ownerPath, 'utf8')).trim();
        } catch {
          return;
        }
        if (owner === token) {
          await fileSystem.rm(generationDirectory, { recursive: true, force: true });
          await fileSystem.rmdir(lockDirectory).catch(() => {});
        }
      };
    } catch (error) {
      if (created) {
        await fileSystem.rm(generationDirectory, { recursive: true, force: true }).catch(() => {});
        await fileSystem.rmdir(lockDirectory).catch(() => {});
      }
      if (error?.code !== 'EEXIST') throw error;

      if (await breakStaleLock(lockDirectory, breakerDirectory, fileSystem)) continue;
...
export async function breakStaleLock(lockDirectory, breakerDirectory, fileSystem = lockFileSystem) {
  try {
    await fileSystem.mkdir(breakerDirectory);
  } catch (error) {
    if (error?.code !== 'EEXIST') throw error;

    try {
      const breaker = await fileSystem.stat(breakerDirectory);
      if (Date.now() - breaker.mtimeMs > 60_000) {
        await fileSystem.rm(breakerDirectory, { recursive: true, force: true });
      }
    } catch {
  ...
  try {
    const generations = (await fileSystem.readdir(lockDirectory, { withFileTypes: true }))
      .find((entry) => entry.isDirectory() && entry.name !== basename(breakerDirectory));
    if (!generations) return false;

    const generationDirectory = join(lockDirectory, generations.name);
    const heartbeatPath = join(generationDirectory, 'heartbeat');
    const details = await fileSystem.stat(heartbeatPath).catch(() => fileSystem.stat(generationDirectory).catch(() => undefined));
    ...
    await fileSystem.rm(breakerDirectory, { recursive: true, force: true }).catch(() => {});
    await fileSystem.rmdir(lockDirectory).catch(() => {});
  }
}
